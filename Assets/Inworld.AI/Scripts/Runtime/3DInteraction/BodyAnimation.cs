...
namespace Inworld.Model
{
    ...
    public class BodyAnimation : MonoBehaviour, InworldAnimation
    {
        ...
        {
            m_InworldCharacter.Emotion = spaffCode.ToString();
            Animator.SetFloat(s_Random, Random.Range(0, 1) > 0.5f ? 1 : 0);
            Animator.SetFloat(s_RemainSec, m_InworldCharacter.CurrentAudioRemainingTime);
            switch (spaffCode)
            {
        ...
        {
            m_InworldCharacter.Gesture = gesture.ToString();
            Animator.SetFloat(s_Random, Random.Range(0, 1) > 0.5f ? 1 : 0);
            Animator.SetFloat(s_RemainSec, m_InworldCharacter.CurrentAudioRemainingTime);
            switch (gesture)
            {
        ...
        void Start()
        {
            ...
            if (m_InworldCharacter && m_InworldCharacter.Audio)
            {
                m_InworldCharacter.Audio.OnAudioStarted += OnAudioStarted;
                m_InworldCharacter.Audio.OnAudioFinished += OnAudioFinished;
            }
        }
        void OnDisable()
        {
        ...
        void OnPacketEvents(InworldPacket packet)
        {
            if (!Animator)
                return;
            if (packet.Routing.Target.Id != m_InworldCharacter.ID && packet.Routing.Source.Id != m_InworldCharacter.ID)
                return;
            switch (packet)
            {
