...
#pragma warning restore CS4014
        ...
        // Collecting audio data from microphone.
        IEnumerator CollectAudioCoroutine()
        {
            while (m_Capture.IsEnabled)
            {
                if (m_Capture.Collect(out ByteString chunk))
                {
                    Routing routing = Routing.FromPlayerToAgent(m_CurrentRecordingID);
                    m_Client.SendAudio(new AudioChunk(chunk, routing));
                }
        ...
        void _StartAudioCapture(string characterID)
        {
            ...
            m_Capture.IsEnabled = true;
            StartCoroutine(CollectAudioCoroutine());
        ...
        IEnumerator InteractionCoroutine()
        {
            while (State == ControllerStates.Connected)
            {
                ...
                _SelectCharacter();
                // Client stopped.
                if (!m_Client.IsInteracting)
                {
                    if (!m_Client.Errors.IsEmpty)
                    {
...
#pragma warning restore 4014
        ...
        public async Task LoadScene(InworldSceneData inworldSceneData)
        {
            ...
            catch (Exception e)
            {
                State = ControllerStates.Error;
                Debug.LogError(e);
                //inworldScene.Event.Invoke(InworldSceneStatus.LoadSceneFailed, null);
            }
        }
        ...
        public void EndAudioCapture(string characterID = null)
        {
            if (string.IsNullOrEmpty(characterID) || characterID.Equals(m_CurrentRecordingID))
            {
                ...
                m_Capture.IsEnabled = false;
                StopCoroutine("CollectAudioCoroutine");
