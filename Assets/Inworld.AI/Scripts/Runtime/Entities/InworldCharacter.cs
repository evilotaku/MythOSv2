...
namespace Inworld
{
    public class InworldCharacter : MonoBehaviour
    {
        ...
        [SerializeField] HeadAnimation m_HeadAnimation;
        [SerializeField] BodyAnimation m_BodyAnimation;
        [Header("Sight")]
        [Range(1, 180)]
        ...
        void Start()
        {
            m_LipAnimation = GetComponentInChildren<ILipAnimations>(); // Yan: Could be Null.
        ...
        void OnAudioFinished()
        {
            ...
            if (m_BodyAnimation)
                m_BodyAnimation.HandleEmotion(EmotionEvent.Types.SpaffCode.Neutral);
            if (m_HeadAnimation)
                m_HeadAnimation.HandleEmotion(EmotionEvent.Types.SpaffCode.Neutral);
        ...
        void OnCharacterChanged(InworldCharacter oldChar, InworldCharacter newChar)
        {
            ...
            {
                if (m_BodyAnimation)
                    m_BodyAnimation.HandleMainStatus(AnimMainStatus.Goodbye);
                _EndAudioCapture();
            }
            else if (newChar == this)
            {
                if (m_BodyAnimation)
                    m_BodyAnimation.HandleMainStatus(AnimMainStatus.Hello);
