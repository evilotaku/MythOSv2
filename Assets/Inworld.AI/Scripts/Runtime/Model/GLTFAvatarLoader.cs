...
namespace Inworld.Model.Sample
{
    ...
    public class GLTFAvatarLoader : MonoBehaviour, IAvatarLoader
    {
        ...
        [SerializeField] GameObject m_LipAnimations;
        ...
        void _SetupLipSync()
        {
            AudioInteraction audioInteraction = m_CharacterToProcess.GetComponent<AudioInteraction>();
            if (!audioInteraction)
            {
                audioInteraction = m_CharacterToProcess.gameObject.AddComponent<AudioInteraction>();
                audioInteraction.Character = m_CharacterToProcess;
            }

            ILipAnimations lipAnimations = m_CharacterToProcess.GetComponentInChildren<ILipAnimations>();
            if (lipAnimations == null)
            {
                GameObject lipAnimObj = Instantiate(m_LipAnimations, m_CharacterToProcess.transform);
                lipAnimations = lipAnimObj.GetComponent<ILipAnimations>();
                AudioSource audioSource = lipAnimObj.GetComponent<AudioSource>();
                audioInteraction.PlaybackSource = audioSource;
            }
            lipAnimations.ConfigureModel(m_CharacterToProcess.CurrentAvatar);
        }
        #endregion
    }
}
